...
int responseClient::checkUri(std::string uri)
{
	struct stat info;

	std::cout << "path = " << uri << std::endl;
	if (stat(uri.c_str(), &info) != 0)
		send_error_status(404);
	else
	{
		file_RW.open(uri);
		if (!file_RW)
		{
			file_RW.open(uri + index);
			if (!file_RW)
...
int responseClient::getMethode(std::string methode)
{
	std::cout << methode << std::endl;
	return 0;
}

int responseClient::postMethode(std::string methode)
{
	std::cout << methode << std::endl;
	return 0;
}

int responseClient::deleteMethode(std::string methode)
{
	std::cout << methode << std::endl;
	return 0;
}
...
int responseClient::ft_response()
{
	...
	for(size_t i=0; i < block_server.size() - 1; i++){
		if ((!block_server[i].server_name.compare(host_serv) || !block_server[i].server.compare(host_serv)) && !block_server[i].port.compare(port_serv))
		{
			root = block_server[i].root;
			index = block_server[i].index;
			//#### matching location
			nBlock = i;
			if (block_server[i].list_of_location.size() == 0)
				return  noLocation();

			for(size_t j=0; j < block_server[i].list_of_location.size() - 1; j++)
			{
				// int k = 0;
			...
			//// methode 
			// methodeFunction[client->request_data_struct->method]("TEST");
